import itertools
from utils.training import cross_task_dataset, compute_eer, compute_auc, add_fft, triplet_loss, remove_unused_modalities, pad_or_slice, downsample
import random, os
...
from utils.config import ds_ratio
...
def training_data_generator_triplets(dataset: dict):
    ...
    while True:
        for i in range(batch_size):
            ...
            # Taking one value each ds_ratio
            genuine_sample = dataset[genuine_subject][genuine_session_idx][task][modality]
            anchor_sample = dataset[genuine_subject][anchor_session_idx][task][modality]
            impostor_sample = dataset[impostor_subject][impostor_session_idx][task][modality]

            # genuine_sample = downsample(genuine_sample, ds_ratio)
            # anchor_sample = downsample(anchor_sample, ds_ratio)
            # impostor_sample = downsample(impostor_sample, ds_ratio)

            # Pad or slice to obtain constant length
            genuine_sample = pad_or_slice(genuine_sample, sequence_len)
            anchor_sample = pad_or_slice(anchor_sample, sequence_len)
            impostor_sample = pad_or_slice(impostor_sample, sequence_len)

            # Adding FFT as additional feature
...
def val_data_generator_triplets(datasets: list, comparisons: dict, labels: dict):
    ...
    while True:
        for i in range(batch_size):
            ...
            genuine_sample = datasets[0][task][genuine_session_idx]
            anchor_sample = datasets[1][task][anchor_session_idx]
            impostor_sample = datasets[1][task][impostor_session_idx]
...
class Predictor_verification(tensorflow.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, x):
        ...
        for task in used_task_list:
            ...
            enrolment_embeddings, verification_embeddings = (self.single_model.predict(enrolment_samples, batch_size=len(enrolment_samples), verbose=0),
                                                             self.single_model.predict(verification_samples, batch_size=len(verification_samples), verbose=0))
